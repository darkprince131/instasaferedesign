...
/* ---------- 4b. the same scenes, for the related-rail thumbnails ----------

   IzRelatedRail has always accepted a `thumb` path and drawn a dashed
   placeholder until real art existed. This is that art: the identical
   drawing the social card carries, so a card in a rail and the image that
   appears when the same page is shared are recognisably one thing.

   Both the SVGs and the route map are emitted HERE rather than copied by
   hand, because two hand-maintained copies of a fifteen-entry list drift
   the first time a scene is renamed, and the failure is silent — the rail
   just quietly falls back to the placeholder on the renamed page. */
const PUB_SCENES = p("public", "og", "scenes");
fs.mkdirSync(PUB_SCENES, { recursive: true });
for (const f of fs.readdirSync(PUB_SCENES)) {
  if (f.endsWith(".svg")) fs.unlinkSync(path.join(PUB_SCENES, f)); // drop art whose scene is gone
}
/* The card and the thumbnail want different crops of the same drawing.
   A social card is 5:4 with air around the subject because it sits beside
   a headline; a rail thumbnail is a 16:9 slot roughly a fifth the size,
   and shown at that crop the artwork became a distant speck floating in
   bone. Each scene carries `data-content` — the ink bounding box inside
   its padded canvas — so the thumbnail is re-framed to 16:9 around the
   ink itself. Nothing is redrawn and nothing is cropped away: only the
   viewBox changes, and the scene's own ground extends past it. */
const THUMB_PAD = 1.08;
for (const s of scenes) {
  const svg = fs.readFileSync(path.join(SCENES, s.file), "utf8");
  const content = /data-content="([-\d.]+) ([-\d.]+) ([-\d.]+) ([-\d.]+)"/.exec(svg);
  if (!content) throw new Error(`${s.file}: no data-content box — cannot frame a thumbnail.`);
  const [x0, y0, x1, y1] = content.slice(1).map(Number);
  const cx = (x0 + x1) / 2;
  const cy = (y0 + y1) / 2;
  let w = (x1 - x0) * THUMB_PAD;
  let h = (y1 - y0) * THUMB_PAD;
  if (w / h < 16 / 9) w = (h * 16) / 9;
  else h = (w * 9) / 16;
  const vb = `viewBox="${(cx - w / 2).toFixed(3)} ${(cy - h / 2).toFixed(3)} ${w.toFixed(3)} ${h.toFixed(3)}"`;
  const framed = svg.replace(/viewBox="[^"]*"/, vb);
  if (framed === svg) throw new Error(`${s.file}: viewBox not rewritten.`);
  fs.writeFileSync(path.join(PUB_SCENES, s.file), framed);
}

const mapBody = scenes
  .map((s) => `  ${JSON.stringify(s.slug)}: "/og/scenes/${s.file}",`)
  .join("\n");
fs.writeFileSync(
  p("lib", "og-scenes.ts"),
